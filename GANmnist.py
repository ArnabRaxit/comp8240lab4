...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        fake = netG(noise)#what is input to netG?
        ...
        ###########################
        netG.zero_grad()
        label.data.fill_(real_label)#which labels are used for the generator?)
        output = netD(fake)
        errG = criterion(output, label)
        errG.backward()#what do we do with the loss before we can take a learning step?
